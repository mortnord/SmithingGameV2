...
public class Sorted_Ingots_Tray : MonoBehaviour, IInteractor_Connector, IData_Transfer
{
    ...
    {
        result = Handle_Quality(main_character.GetComponent<DwarfScript>().Get_Item_In_Inventory().GetComponent<Common_Properties>().Get_Ore_Quality());
        if (result == true)
        {
            ...
            Ingots_in_tray.Add(main_character.GetComponent<DwarfScript>().Get_Item_In_Inventory());
    ...
    public void Pickup(GameObject main_character) //Her henter vi opp ting fra stockpilen. 
    {
        ...
        {
            main_character.GetComponent<DwarfScript>().Set_Item_In_Inventory(Ingots_in_tray[0]);
