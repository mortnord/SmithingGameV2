...
public class Anvil : MonoBehaviour, IInteractor_Connector, IInteract_Work
{
    ...
    public void Pickup(GameObject main_character) //Interface metode for å hente ut converted objekt først, så prøver den å rydde opp i blueprint copien
    {
        ...
        {
            main_character.GetComponent<DwarfScript>().Set_Item_In_Inventory(Converted_Object);
        ...
        {
            main_character.GetComponent<DwarfScript>().Set_Item_In_Inventory(blueprint_copy);
            blueprint_copy = null;
            Return_Answer(main_character, true); //Returnerer svar at vi fikk plukket oppe noe
        }
        else if (object_to_be_destroyed != null) //hvis ikke, hent råmaterialet
        {
            main_character.GetComponent<DwarfScript>().Set_Item_In_Inventory(object_to_be_destroyed);
    ...
    public void Drop_Off(GameObject main_character) //Interface metode for å legge inn objekt. 
    {
        ...
        {
            object_to_be_destroyed = main_character.GetComponent<DwarfScript>().Get_Item_In_Inventory(); //Legger inn og flytter råmateriale på ambolten
        ...
        {
            blueprint_copy = main_character.GetComponent<DwarfScript>().Get_Item_In_Inventory(); 
            main_character.GetComponent<DwarfScript>().Get_Item_In_Inventory().transform.position = gameObject.transform.position;
            Return_Answer(main_character, false); //Returnerer svar at vi fikk lagt fra oss noe.
        }
    }
