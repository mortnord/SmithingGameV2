...
using UnityEngine;
public class Furnace : MonoBehaviour, IInteractor_Connector, IData_Transfer
{
    ...
    void Update()
    {
        ...
        if (smelting_time < 1) //Når det har gått 10 sekund etter at vi la inn første malmen, så smeltes den til en barre. 
        {
            ...
            {
                smelting_quality = Find_Quality(Ores_in_furnace[0]);
                ...
                {
                    liquid_metal += Get_Amount_Of_Metal(Ores_in_furnace[0]);
                    Destroy(Ores_in_furnace[0]);
                    Ores_in_furnace.RemoveAt(0);
                }
            }
    ...
    private float Get_Amount_Of_Metal(GameObject item_in_inventory)
    {
        int liquids = 0;
        if (item_in_inventory.GetComponent<Ore>() != null) //Hvis ore, sjekk kvalitet og bruk det som input. 
        {
            liquids = item_in_inventory.GetComponent<Ore>().Get_Percent_Ore_To_Ingot();
        }
    ...
    public int Find_Quality(GameObject item_in_inventory)
    {
        if (item_in_inventory.GetComponent<Common_Properties>() != null) //Hvis ore, sjekk kvalitet og bruk det som input. 
        {
            quality_in_input = (int)item_in_inventory.GetComponent<Common_Properties>().Get_Ore_Quality();
        }
        else
        {
            quality_in_input = -1;
    ...
    {
        result = Handle_Quality(Find_Quality(main_character.GetComponent<MainCharacterStateManager>().Get_Item_In_Inventory()));
        if (result == true)
        {
            ...
            Ores_in_furnace.Add(main_character.GetComponent<MainCharacterStateManager>().Get_Item_In_Inventory());
            Return_Answer(main_character, false);
        }
    }
    private bool Handle_Quality(int ore_quality) //Her setter vi kvaliteten som skal være i furnacen, dette hindrer 2 typer malm å være inni samtidig.
    {
        if (Ores_in_furnace.Count == 0)
        {
    ...
    public void Storage() //Her lagrer vi malmen som er i furnacen. 
    {
        for (int i = 0; i < Ores_in_furnace.Count; i++) //Generer ore og legg det i stockpilen for usortert ore
        {
            ...
            {
                StaticData.furnace_quality_static_object.Add(Ores_in_furnace[i].GetComponent<Common_Properties>().Get_Ore_Quality());
                StaticData.percent_ore_quality_furnace.Add(Ores_in_furnace[i].GetComponent<Ore>().Get_Percent_Ore_To_Ingot());
            }
    ...
    public void Loading() //Last inn data. 
    {
        if (StaticData.furnace_quality_static_object.Count > 0)
        {
            for (int i = 0; i < StaticData.furnace_quality_static_object.Count; i++) //Generer ore og legg det i stockpilen for usortert ore
            {
                ...
                Ores_in_furnace[i].GetComponent<Common_Properties>().ore_quality = (Enumtypes.Ore_Quality)Ores_in_furnace[i].GetComponent<Common_Properties>().Get_Ore_Quality();
